...
namespace InventoryManagementSystem
{
    ...
    {
        SqlConnection con = new SqlConnection(@"Data Source=(LocalDB)\MSSQLLocalDB;AttachDbFilename=C:\Users\new owner\Documents\mydata.mdf;Integrated Security=True;Connect Timeout=30");
        ...
        public void LoadCategory()
        {
            ...
            cm = new SqlCommand("SELECT catname FROM Category", con);
            con.Open();
            dr = cm.ExecuteReader();
            while (dr.Read())
            {
        ...
        private void btnSave_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (MessageBox.Show("Are you sure you want to save this product?", "Saving Record", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
                {
                    ...
                    cm = new SqlCommand("INSERT INTO tbProduct(pname,pqty,pprice,pdescription,pcategory,pid)VALUES(@pid,@pname, @pqty, @pprice, @pdescription, @pcategory)", con);
        ...
        private void btnUpdate_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (MessageBox.Show("Are you sure you want to update this product?", "Update Record", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
                {
                    ...
                    cm = new SqlCommand("UPDATE tbProduct SET pid=@pid pname = @pname, pqty=@pqty, pprice=@pprice, pdescription=@pdescription, pcategory=@pcategory WHERE pid LIKE '" + lblPid.Text + "' ", con);
                    cm.Parameters.AddWithValue("@pid", txtpid.Text);
                    cm.Parameters.AddWithValue("@pname", txtPName.Text);
                    cm.Parameters.AddWithValue("@pqty", Convert.ToInt16(txtPQty.Text));
                    cm.Parameters.AddWithValue("@pprice", Convert.ToInt16(txtPPrice.Text));
